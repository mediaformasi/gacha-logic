...
// Panggil kartu
const card_list = require('./card-list.json');

// Rate dari gachanya, terserah pengen isi apa nilainya.
// Ini ratenya dalam bentuk persen.
const r_rate = 90;
const sr_rate = 8.1;
const ssr_rate = 1.9;
...
// Mari kita mencetak datanya
// Buat variabel resources sebagai penampung database sementara
let res = searchBasedWeight(card_list, r_rate, sr_rate, ssr_rate, event_rate, true);

// Penggunaan metode perulangan untuk mencetak kodenya
// Nanti ekspektasinya outputnya bakal kek gini:
// You've got {name} ({rarity}) [{on_event} CARD] [{costume} Costume] ID:{id}
// Dan dicetak sebanyak 10x karena kita sebut tadi one_or_ten = true
for (let i = 0; i < res.length; i++) {
    // Karena resourcesnya adalah array, kita panggil dengan cara:
    // array[urutan].<property>
    let resID = res[i].id;
    let resName = res[i].name;
    let resRarity = res[i].rarity;
    let resCostume = res[i].costume;
    // Oh iya, liat kode untuk resOnEvent ini, agak aneh kan kelihatan.
    // Perkenalkan, ini namanya IF ELSE One Line, update ES2015.
